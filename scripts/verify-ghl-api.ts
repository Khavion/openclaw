...
async function writeReport(): Promise<void> {
  ...
  // Preserve the doc-verified scope-architecture sections at the top of the
  // report; only the live-results tail is regenerated per run.
  const MARKER = '## Live endpoint results';
  let preamble = `# Stage 0 GHL API Verification Report\n\n`;
  try {
    const existing = await (await import('node:fs/promises')).readFile(REPORT_PATH, 'utf8');
    const idx = existing.indexOf(MARKER);
    if (idx !== -1) preamble = existing.slice(0, idx);
  } catch {
    /* first run without the pre-run report; write standalone */
  }
  const lines: string[] = [preamble.trimEnd(), '', MARKER, ''];
